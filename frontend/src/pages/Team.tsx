  ...
  return (
    <Layout title="Gerenciamento de Equipe">
      <div className="space-y-8">
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm">
              <div className="p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/20 flex justify-between items-center">
                <div>
                  <h3 className="text-base font-bold text-slate-900 dark:text-white">Garçons & Caixas (Login por PIN)</h3>
                  <p className="text-slate-600 dark:text-slate-400 text-xs mt-0.5">{operationalStaff.length} funcionário(s) cadastrado(s)</p>
                </div>
              </div>
              ...
              {operationalStaff.length === 0 ? (
                <div className="text-center py-14 px-6">
                  ...
                  <p className="text-slate-600 dark:text-slate-400 font-bold">Nenhum funcionário operacional cadastrado ainda.</p>
                ...
                <div className="divide-y divide-slate-800/50">
                  {operationalStaff.map(member => (
                    <motion.div
                      key={member.id}
                      layout
                      className="flex items-center justify-between px-6 py-4 hover:bg-slate-100 dark:bg-slate-800/20 transition-colors"
                    >
                      ...
                      <div className="flex items-center gap-2">
                        {/* PIN indicator */}
                        <div className="hidden sm:flex items-center gap-1 text-xs text-slate-500 font-mono bg-slate-50 dark:bg-slate-950/60 px-3 py-1.5 rounded-lg border border-slate-850">
                        ...
                        <button
                          ...
                          className="p-2 text-slate-500 hover:text-indigo-400 bg-slate-50 dark:bg-slate-950/60 border border-slate-850 rounded-xl transition-colors cursor-pointer"
                          title="Alterar PIN"
                        >
                        ...
                        <button
                          ...
                          disabled={actionLoading}
                          className="p-2 text-slate-500 hover:text-red-400 bg-slate-50 dark:bg-slate-950/60 border border-slate-850 rounded-xl transition-colors cursor-pointer"
            ...
            {managers.length > 0 && (
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm">
                <div className="p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/20">
                  <h3 className="text-base font-bold text-slate-900 dark:text-white">Gerentes (Login por E-mail + Senha)</h3>
                  <p className="text-slate-600 dark:text-slate-400 text-xs mt-0.5">Para gerenciar gerentes, acesse Administração → Usuários</p>
                </div>
                <div className="divide-y divide-slate-800/50">
                  {managers.map(m => (
                    <div key={m.id} className="flex items-center gap-4 px-6 py-4">
                      ...
                      <div>
                        <p className="text-slate-900 dark:text-white font-bold text-sm">{m.name}</p>
        ...
        {/* Bottom Actions */}
        <div className="pt-8 pb-12 flex justify-center sm:justify-end border-t border-slate-200 dark:border-slate-800/50 mt-8">
          <button 
            onClick={() => {
      ...
      <AnimatePresence>
        {isCreateOpen && (
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-50 dark:bg-slate-950/80 backdrop-blur-sm">
            <motion.div
              initial={{ opacity: 0, y: 60 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 60 }}
              transition={{ type: 'spring', damping: 25, stiffness: 300 }}
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl max-h-[90vh] flex flex-col"
            >
              {/* Modal Header */}
              <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/30">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 bg-indigo-500/10 rounded-xl">
                    <UserCheck className="w-5 h-5 text-indigo-400" />
                  </div>
                  <div>
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white">Novo Funcionário</h3>
                    <p className="text-slate-600 dark:text-slate-400 text-xs">Acesso rápido via PIN numérico</p>
                  </div>
                </div>
                <button onClick={() => setIsCreateOpen(false)} className="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors cursor-pointer">
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <form onSubmit={handleCreateStaff} className="p-6 space-y-5 overflow-y-auto custom-scrollbar">
                ...
                {/* Name */}
                <div>
                  <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-2">Nome Completo</label>
                  <input
                    type="text"
                    required
                    value={formName}
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white rounded-xl px-4 py-3.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none"
                  />
                </div>

                {/* Role */}
                <div>
                  <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-2">Função / Cargo</label>
                      ...
                      <button
                        ...
                        className={`flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all cursor-pointer ${
                          formRole === opt.value
                            ...
                            : 'border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/40 hover:border-slate-300 dark:border-slate-700'
                        }`}
                      >
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider">PIN de Acesso</label>
                    <button
                      type="button"
                        ...
                        {['1','2','3','4','5','6','7','8','9','','0','del'].map((k, i) => (
                          <button
                            ...
                            className={`h-14 rounded-xl font-bold text-lg transition-all cursor-pointer ${
                              !k ? 'invisible' :
                              k === 'del'
                                ? 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-red-500/10 hover:text-red-400 active:scale-95'
                                : 'bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white hover:bg-slate-700 active:scale-95 shadow-sm'
                            }`}
                          >
                            {k === 'del' ? '⌫' : k}
                          </button>
                    ...
                    <div className="space-y-3">
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white rounded-xl py-3.5 pl-10 pr-12 text-sm focus:ring-2 focus:ring-indigo-500 outline-none tracking-[0.4em] font-bold"
                        />
                        <button
                          type="button"
                          onClick={() => setShowPin(p => !p)}
                          className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-900 dark:text-white transition-colors cursor-pointer"
                        >
                      ...
                      <input
                        ...
                        className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white rounded-xl py-3.5 px-4 text-sm focus:ring-2 focus:ring-indigo-500 outline-none tracking-[0.4em] font-bold"
                      />
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    ...
                    className="flex-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-700 text-slate-900 dark:text-white font-bold py-3.5 rounded-xl transition-colors cursor-pointer"
                  >
                    Cancelar
                  </button>
                  <button
        ...
        {editingId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-950/80 backdrop-blur-sm">
            <motion.div
              ...
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-xs rounded-3xl overflow-hidden shadow-2xl"
            >
              <div className="flex items-center justify-between p-5 border-b border-slate-200 dark:border-slate-800">
                <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                  <Hash className="w-5 h-5 text-indigo-400" /> Alterar PIN
                </h3>
                <button onClick={() => setEditingId(null)} className="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white cursor-pointer">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <form onSubmit={handleUpdatePin} className="p-5 space-y-4">
                <div>
                  <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-2">Novo PIN</label>
                  <input
                    type="password"
                    inputMode="numeric"
                    pattern="\d*"
                    ...
                    placeholder="Mínimo 3 dígitos"
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none tracking-[0.4em] font-bold"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-2">Confirmar PIN</label>
                  <input
                    type="password"
                    inputMode="numeric"
                    pattern="\d*"
                ...
                <div className="flex gap-3">
                  <button type="button" onClick={() => setEditingId(null)} className="flex-1 bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white font-bold py-3 rounded-xl cursor-pointer">Cancelar</button>
