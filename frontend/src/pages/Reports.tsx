  ...
  return (
    <Layout title="Relatórios e Análises">
      <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-md">
          {/* Tabs */}
          <div className="flex bg-slate-50 dark:bg-slate-950 p-1 border border-slate-850 rounded-2xl w-fit">
            {isPro && (
              <button
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex bg-slate-50 dark:bg-slate-950 p-1 border border-slate-850 rounded-2xl">
            ...
            {period === 'custom' && (
              <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-950 p-1.5 border border-slate-850 rounded-2xl animate-fade-in">
                <Calendar className="w-4 h-4 text-slate-500 ml-2" />
                <input 
                  type="date" 
                  value={startDate}
                  onChange={e => setStartDate(e.target.value)}
                  className="bg-transparent border-none text-xs text-slate-900 dark:text-white outline-none focus:ring-0 cursor-pointer"
                />
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl gap-3">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
                    <div>
                      ...
                      <h3 className="text-2xl font-black text-slate-900 dark:text-white">{formatBRL(cmvSummary?.total_revenue)}</h3>
                    </div>
                  ...
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
                    <div>
                      ...
                      <h3 className="text-2xl font-black text-slate-900 dark:text-white">{formatBRL(cmvSummary?.total_cost)}</h3>
                    </div>
                  ...
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
                    <div>
                      ...
                      <h3 className="text-2xl font-black text-slate-900 dark:text-white">{formatBRL(cmvSummary?.total_profit)}</h3>
                    </div>
                  ...
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
                    <div>
                      ...
                      <h3 className="text-2xl font-black text-slate-900 dark:text-white">{cmvSummary?.margin_pct}%</h3>
                    </div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl">
                  <div className="flex justify-between items-center mb-6">
                    <div>
                      <h3 className="text-lg font-bold text-slate-900 dark:text-white">Análise CMV Detalhada por Item</h3>
                      <p className="text-sm text-slate-600 dark:text-slate-400">Relação de custos, receitas e margem percentual por produto</p>
                    </div>
                    <button 
                      onClick={() => exportCSV(cmvProducts, 'relatorio_cmv_star_food')}
                      className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white font-bold py-2.5 px-4 rounded-xl flex items-center gap-2 text-xs transition-colors cursor-pointer"
                    >
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="border-b border-slate-200 dark:border-slate-800 text-slate-500 text-xs font-black uppercase tracking-wider">
                      ...
                      <tbody className="divide-y divide-slate-850 text-slate-700 dark:text-slate-300">
                        {cmvProducts.length === 0 ? (
                          ...
                          cmvProducts.map((item) => (
                            <tr key={item.id} className="hover:bg-slate-50 dark:bg-slate-950/40 transition-colors">
                              <td className="py-4 font-bold text-slate-900 dark:text-white text-sm">{item.name}</td>
                              ...
                              <td className="py-4 text-right font-mono text-sm text-slate-600 dark:text-slate-400">{formatBRL(item.total_cost)}</td>
                              <td className="py-4 text-right font-mono text-sm text-emerald-400 font-bold">{formatBRL(item.gross_profit)}</td>
                              <td className="py-4 text-center">
                                <span className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 px-2 py-0.5 rounded font-mono text-xs">
                                  {item.cmv_pct}%
                                </span>
                              </td>
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {topProducts.slice(0, 3).map((item, idx) => {
                    ...
                    const medalColors = ['text-yellow-500', 'text-slate-700 dark:text-slate-300', 'text-amber-600'];

                    return (
                      <div 
                        key={item.id} 
                        className={`bg-white dark:bg-slate-900 border rounded-3xl p-6 shadow-lg bg-gradient-to-br ${bgGradients[idx]}`}
                      >
                        ...
                        <div className="space-y-1.5 text-sm text-slate-600 dark:text-slate-400">
                          <div className="flex justify-between">
                            ...
                            <strong className="text-slate-900 dark:text-white font-mono">{Number(item.qty_sold)} un.</strong>
                          </div>
                          <div className="flex justify-between">
                            <span>Faturamento Total:</span>
                            <strong className="text-slate-900 dark:text-white font-mono">{formatBRL(item.revenue)}</strong>
                          </div>
                          <div className="flex justify-between">
                            <span>Preço Médio:</span>
                            <strong className="text-slate-900 dark:text-white font-mono">{formatBRL(item.price)}</strong>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  {topProducts.length === 0 && (
                    <div className="md:col-span-3 text-center py-12 text-slate-500 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl">
                ...
                {topProducts.length > 0 && (
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl">
                    <div className="flex justify-between items-center mb-6">
                      <div>
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white">Gráfico de Saída e Penetração</h3>
                        <p className="text-sm text-slate-600 dark:text-slate-400">Quantidade de itens vendidos em relação ao líder do ranking</p>
                      </div>
                      <button 
                          ...
                          <div key={prod.id} className="group">
                            <div className="flex justify-between items-end mb-2">
                              <div className="flex items-center gap-3">
                                <span className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 w-7 h-7 rounded-lg flex items-center justify-center text-xs font-black font-mono shrink-0">
                                  {idx + 1}
                                </span>
                                <span className="text-sm font-bold text-slate-900 dark:text-white group-hover:text-indigo-400 transition-colors">{prod.name}</span>
                              </div>
            ...
            {activeTab === 'export' && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm hover:border-indigo-500/50 transition-colors cursor-pointer group">
                  ...
                  <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Exportar Vendas (CSV)</h3>
                  <p className="text-slate-600 dark:text-slate-400 text-sm mb-6">Gera uma planilha de auditoria completa de pedidos finalizados com data, garçom, mesa e total líquido.</p>
                  <button 
                    onClick={async () => {
                      try {
                        const dates = getDates();
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-white font-medium py-3 rounded-xl flex items-center justify-center gap-2 group-hover:bg-indigo-500 group-hover:border-indigo-500 transition-colors cursor-pointer"
                  >
                ...
                {isPro && (
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm hover:border-emerald-500/50 transition-colors cursor-pointer group">
                    <div className="w-14 h-14 bg-emerald-500/10 rounded-2xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                      <Percent className="w-7 h-7 text-emerald-500" />
                    </div>
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Exportar Margem CMV</h3>
                    <p className="text-slate-600 dark:text-slate-400 text-sm mb-6">Planilha contendo a quebra detalhada do Custo de Mercadoria Vendida (CMV) por produto para o seu time contábil.</p>
                    <button 
                      onClick={async () => {
                        try {
                          const dates = getDates();
                      ...
                      className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-white font-medium py-3 rounded-xl flex items-center justify-center gap-2 group-hover:bg-emerald-500 group-hover:border-emerald-500 transition-colors cursor-pointer"
                    >
                      <Download className="w-4 h-4" /> Baixar Planilha
                    </button>
                  </div>
