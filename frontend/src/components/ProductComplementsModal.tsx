          ...
          >
            ...
            <div className="p-6 overflow-y-auto flex-1">
              ...
              <div className="mb-6 bg-slate-950 p-4 rounded-xl border border-slate-800">
                <label className="block text-xs font-bold text-slate-400 mb-2">Vincular Novo Grupo</label>
                <div className="flex gap-2">
                  <select
                    value={selectedCatId}
                    onChange={(e) => setSelectedCatId(e.target.value)}
                    className="flex-1 bg-slate-900 border border-slate-800 text-white rounded-lg px-3 py-2 outline-none focus:border-indigo-500"
                  >
              ...
              <div>
                <h4 className="text-sm font-bold text-slate-300 mb-3">Grupos Vinculados</h4>
                {loading && linkedCategories.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {linkedCategories.map(cat => (
                      <div key={cat.id} className="flex justify-between items-center bg-slate-950 border border-slate-800 p-3 rounded-xl">
